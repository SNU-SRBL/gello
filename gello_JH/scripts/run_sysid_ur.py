...
# SIMPLER-Style System Identification

"""SIMPLER-Style System Identification for UR5e + Tesollo.

Uses EE pose tracking loss (L_transl + L_rot) to calibrate UR5e PD parameters.
Executes action trajectories in open-loop and compares Real vs Sim EE poses.

Loss Functions (SIMPLER Eq. 3-5):
    L_transl = (1/T) * sum(||x_real - x_sim||^2)
    L_rot = (1/T) * sum(geodesic_distance(R_real, R_sim))
...
Usage:
    ./isaaclab.sh -p scripts/run_sysid_simpler.py \
        --trajectory_dir data/lerobot/HD_0204 \
        --output_dir results/sysid_simpler \
        --num_rounds 3 \
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="SIMPLER-Style System Identification for UR5e"
    )
    ...
    # USD defaults: UR5e K=[1.08, 1023.22], D=[0.004, 4.09]
    # These defaults correspond to USD K=[5, 1500], D=[0.01, 10].
    parser.add_argument(
        "--stiffness_min",
        type=float,
        default=286.5,
        help="Minimum stiffness bound in PhysX units (≈ USD 5.0)",
    )
    parser.add_argument(
        "--stiffness_max",
        type=float,
        default=85944.0,
        help="Maximum stiffness bound in PhysX units (≈ USD 1500.0)",
    )
    parser.add_argument(
        "--damping_min",
        type=float,
...
def main():
    ...
    # Position: apply Rz(180°) after world→sim_base → negate x, y
    # Rotation: apply similarity transform Rz180 * q * conj(Rz180)
    ...
    def quat_multiply(q1, q2):
        """Hamilton product q1 * q2, quaternion [w, x, y, z]."""
        w1, x1, y1, z1 = q1
        w2, x2, y2, z2 = q2
        return np.array([
    ...
    def world_to_base_pos(pos_w, base_pos_w, base_quat_w):
        """Transform position from world frame to UR5e base frame."""
        # World → sim base frame
        pos_base = quat_rotate_inverse(base_quat_w, pos_w - base_pos_w)
        # Sim base → UR5e base (Rz180: negate x, y)
        return np.array([-pos_base[0], -pos_base[1], pos_base[2]])

    def world_to_base_quat(quat_w, base_quat_w):
        """Transform quaternion from world frame to UR5e base frame."""
        # World → sim base frame
        quat_base = quat_multiply(quat_conjugate(base_quat_w), quat_w)
        # Sim base → UR5e base: similarity transform Rz180 * q * conj(Rz180)
        return quat_multiply(quat_multiply(RZ180, quat_base), RZ180_CONJ)

    print("=" * 70)
    print("SIMPLER-Style System Identification for UR5e")
    print("=" * 70)
    ...
    print(f"  Real EE pos shape: {real_ee_pos.shape}")
    print(f"  Real EE rot shape: {real_ee_rot.shape}")
    ...
    EE_BODY_NAME = "wrist_3_link"  # UR5e end-effector link
    ...
    print(f"  UR5e joints: {UR5E_JOINTS}")
    print(f"  EE body: {EE_BODY_NAME}")
    ...
    # Capture robot base frame in world coordinates (for world→base transform)
    # Rz(180°) correction is applied inside world_to_base_pos/quat functions
    base_pos_w = robot.data.root_pos_w[0].cpu().numpy()
    base_quat_w = robot.data.root_quat_w[0].cpu().numpy()  # wxyz
    print(f"  Robot base pos (world): [{base_pos_w[0]:.6f}, {base_pos_w[1]:.6f}, {base_pos_w[2]:.6f}]")
    print(f"  Robot base quat (world): [{base_quat_w[0]:.6f}, {base_quat_w[1]:.6f}, {base_quat_w[2]:.6f}, {base_quat_w[3]:.6f}]")

    # Pre-compute initial poses with WRIST3_OFFSET
    ur5e_init = ur5e_joint_pos[0].copy()
    ur5e_init[5] += WRIST3_OFFSET

    # ===== Step 4.5: Initial EE Position Analysis =====
    print("\n[Step 4.5] Initial EE position analysis (same joint angles)...")

    # Set initial joint positions from dataset
    ...
    # Compare sim EE vs real EE at t=0 (both in robot base frame)
    sim_ee_pos_w = robot.data.body_pos_w[0, ee_body_idx, :].cpu().numpy()
    sim_ee_rot_w = robot.data.body_quat_w[0, ee_body_idx, :].cpu().numpy()
    sim_ee_pos_init = world_to_base_pos(sim_ee_pos_w, base_pos_w, base_quat_w)
    sim_ee_rot_init = world_to_base_quat(sim_ee_rot_w, base_quat_w)
    real_ee_pos_init = real_ee_pos[0]
    real_ee_rot_init = real_ee_rot[0]

    diff_pos = sim_ee_pos_init - real_ee_pos_init
    diff_pos_norm = np.linalg.norm(diff_pos)

    print(f"  Real  EE pos (t=0, base frame): [{real_ee_pos_init[0]:.6f}, {real_ee_pos_init[1]:.6f}, {real_ee_pos_init[2]:.6f}]")
    print(f"  Sim   EE pos (t=0, base frame): [{sim_ee_pos_init[0]:.6f}, {sim_ee_pos_init[1]:.6f}, {sim_ee_pos_init[2]:.6f}]")
    print(f"  Diff  (sim - real): [{diff_pos[0]:.6f}, {diff_pos[1]:.6f}, {diff_pos[2]:.6f}]")
    print(f"  Diff  norm:          {diff_pos_norm:.6f} m  ({diff_pos_norm * 1000:.2f} mm)")
    print(f"  Real  EE rot (t=0): {real_ee_rot_init}")
    print(f"  Sim   EE rot (t=0): {sim_ee_rot_init}")

    # Also print joint positions for comparison
    sim_joint_pos_init = robot.data.joint_pos[0].cpu().numpy()
    print(f"  Real  joint pos (t=0): {ur5e_joint_pos[0]}")
    print(f"  Sim   joint pos (t=0): {[sim_joint_pos_init[idx] for idx in ur5e_indices]}")

    # ===== Step 5: Define Loss Function =====
    print("\n[Step 5] Defining SIMPLER loss function...")

    trial_counter = [0]

    def compute_sysid_loss(params: dict[str, np.ndarray]) -> float:
        """Compute SIMPLER-style SysID loss for given PD parameters."""
        ...
        try:
            ...
            # Apply PD parameters to UR5e joints
            ...
            # Run open-loop trajectory, record EE poses
            sim_ee_pos_traj = np.zeros((max_frames, 3))
            sim_ee_rot_traj = np.zeros((max_frames, 4))
            ...
            for t in range(max_frames):
                ...
                # Record EE pose (convert world → base frame)
                ee_pos_w = robot.data.body_pos_w[0, ee_body_idx, :].cpu().numpy()
                ee_rot_w = robot.data.body_quat_w[0, ee_body_idx, :].cpu().numpy()
                sim_ee_pos_traj[t] = world_to_base_pos(ee_pos_w, base_pos_w, base_quat_w)
                sim_ee_rot_traj[t] = world_to_base_quat(ee_rot_w, base_quat_w)

            # Compute SIMPLER loss
            total_loss, loss_components = simpler_sysid_loss(
                real_ee_pos=real_ee_pos[:max_frames],
                sim_ee_pos=sim_ee_pos_traj,
                real_ee_rot=real_ee_rot[:max_frames],
                sim_ee_rot=sim_ee_rot_traj,
            )
    ...
    param_bounds = {
        "stiffness": (
            np.full(6, args.stiffness_min),
            np.full(6, args.stiffness_max),
        ),
        "damping": (
            np.full(6, args.damping_min),
            np.full(6, args.damping_max),
        ),
    }
    ...
    # SA will sample random initial params within bounds
    best_params = optimizer.optimize(
        loss_fn=compute_sysid_loss,
    ...
    final_loss = compute_sysid_loss(best_params)
    ...
    # Run trajectory
    sim_ee_pos_final = np.zeros((max_frames, 3))
    sim_ee_rot_final = np.zeros((max_frames, 4))
    ...
    for t in range(max_frames):
        ...
        ee_pos_w = robot.data.body_pos_w[0, ee_body_idx, :].cpu().numpy()
        ee_rot_w = robot.data.body_quat_w[0, ee_body_idx, :].cpu().numpy()
        sim_ee_pos_final[t] = world_to_base_pos(ee_pos_w, base_pos_w, base_quat_w)
        sim_ee_rot_final[t] = world_to_base_quat(ee_rot_w, base_quat_w)

    final_total, final_components = simpler_sysid_loss(
        real_ee_pos=real_ee_pos[:max_frames],
        sim_ee_pos=sim_ee_pos_final,
        real_ee_rot=real_ee_rot[:max_frames],
        sim_ee_rot=sim_ee_rot_final,
    )
    ...
    result_file = output_dir / "sysid_simpler_result.yaml"
