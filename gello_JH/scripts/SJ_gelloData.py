...
class SJ_GelloDataProcessor:
    ...
    def _data_parser(self, filename, file):
        ...
        timestr = timestr.replace('_', ':').replace(' ', ':')
    ...
    def plot_time(self, save_path=None):
        ...
        # Unit mapping for each data type
        units = {
            'joint_positions': 'rad',
            'robot_velocity': 'rad/s',
            'robot_current': 'A',
            'finger_positions': 'deg',
            'finger_velocity': 'RPM',
            'finger_current': 'A',
            'fingertip_sensor': 'F×0.1=N, T=mNm',
            'control': 'rad',
        }
        ...
        for key, values in data.items():
            if key != 'time':
                ...
                unit = units.get(key, '')
                ax[r, c].set_ylabel(f'[{unit}]' if unit else '')
...
if __name__ == "__main__":
    ...
    print(f"Hz: {data_processor.get_Hz():.2f}")

    # Save plot to Real_data folder
    save_path = os.path.join(data_directory, f"{target_data_directory}_plot.png")
    data_processor.plot_time(save_path=save_path)